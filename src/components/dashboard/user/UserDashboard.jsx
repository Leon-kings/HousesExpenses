  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const responses = await Promise.allSettled([
        ...
        // Recent Transactions - specifically from expenses by email
        email ? axios.get(`${API_BASE_URL}/expenses/email/${email}`, {
          headers: { Authorization: `Bearer ${token}` },
        }) : Promise.reject("No email found"),
      ]);
      ...
      // Recent Expenses (by email)
      if (responses[7].status === "fulfilled") {
        console.log("Recent Expenses by Email API Response:", responses[7].value.data);
        recentExpenses = responses[7].value.data?.data || [];
      } else {
        console.error("Recent Expenses by Email API Error:", responses[7].reason);
        toast.warning("Could not load recent expenses data");
      }
      ...
      // Get recent transactions from expenses by email
      const recentTrans = recentExpenses.map((e) => ({
        ...e,
        type: "expense",
      }));

      console.log("Recent transactions from expenses by email:", recentTrans);
      setAllTransactions(recentTrans);

      // Get recent transactions for stats (latest 8)
      const recentTransactions = recentTrans.slice(0, 8);
