...
//           type="submit"
//           disabled={isSubmitting}
//           className="w-full xs:w-auto px-6 py-2 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2 text-sm sm:text-base"
//         >
//           {isSubmitting ? (
//             <>
//               <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
...
//           ) : (
//             <>
//               <AttachMoneyIcon className="w-4 h-4 sm:w-5 sm:h-5" />
...
//         className={`bg-white rounded-2xl shadow-lg p-4 sm:p-6 border-l-4 transition-all hover:shadow-xl ${
//           isCompleted
//             ? "border-l-green-500"
//             : progress >= 75
//               ? "border-l-blue-500"
...
//               <span className="truncate">{saving.category}</span>
//               {isCompleted && (
...
//           <div className="flex flex-wrap justify-between text-xs sm:text-sm gap-1">
...
//           <div className="flex flex-wrap justify-between text-xs text-gray-500 gap-1">
...
//         <div className="flex flex-wrap items-center justify-between gap-2 mt-3 sm:mt-4 pt-3 border-t border-gray-100">
...
//   // Load savings from API using /email/:email endpoint
...
//       const url = `/savings/email/${user.email}`;
//       const params = {};
...
//     a.href = url;
//     a.download = `my-savings-report-${new Date().toISOString().split("T")[0]}.csv`;
//     a.click();
...
//               <SavingsIcon className="text-blue-500 text-2xl sm:text-3xl" />
//               My Savings
//             </h2>
...
//               className="flex items-center space-x-1 sm:space-x-2 px-3 sm:px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-all duration-200 shadow-md hover:shadow-lg text-sm sm:text-base"
//             >
...
//                 loadSavings();
//               }}
//               className="flex items-center space-x-1 sm:space-x-2 px-3 sm:px-4 py-2 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-all duration-200 text-sm sm:text-base"
//             >
...
//             </p>
//           </div>
//           <div className="bg-white rounded-xl shadow-lg p-3 sm:p-4 border-l-4 border-emerald-500">
...
//             <SavingsIcon className="w-16 h-16 sm:w-20 sm:h-20 text-gray-300 mx-auto mb-4" />
...
//           <div className="flex flex-col xs:flex-row justify-center gap-2 xs:gap-3 mt-6">
//             <button
//               onClick={() => {
//                 setIsDeleteModalOpen(false);
//                 setSelectedSavings(null);
...
//               disabled={isSubmitting}
//               className="px-6 py-2 bg-red-500 text-white rounded-lg shadow-lg hover:bg-red-600 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
//             >
//               {isSubmitting ? (
//                 <>
//                   <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
...
//   // CRITICAL: Load savings and filter by user name
//   const loadSavings = useCallback(async () => {
//     // Prevent concurrent loads
//     if (isLoadingRef.current) return;
...
//       // Fetch ALL savings from the API
...
//         console.log(`✅ Found ${allSavings.length} total savings goals (direct array)`);
//       } else {
//         toast.warning("Unexpected response format");
//         setIsLoading(false);
//         isLoadingRef.current = false;
...
//       // CRITICAL: Filter savings by user name (case insensitive)
//       let filteredData = allSavings;

//       if (!isAdmin && targetUserName) {
//         filteredData = allSavings.filter(s => {
//           const savingUserName = s.userName || s.user || "";
//           return savingUserName.toLowerCase() === targetUserName.toLowerCase();
//         });
//         console.log(`✅ Filtered to ${filteredData.length} savings goals for user: "${targetUserName}"`);
//       } else if (isAdmin) {
//         // Admin can see all savings, but can filter by user name
//         if (filterUserName) {
...
//           });
//           console.log(`✅ Filtered to ${filteredData.length} savings goals containing user name: "${filterUserName}"`);
//         } else {
//           console.log(`👑 Admin viewing all ${filteredData.length} savings goals`);
//         }
//       }
...
//       isLoadingRef.current = false;
//     }
//   }, [user, isAdmin, userName, filterUserName, navigate]);
