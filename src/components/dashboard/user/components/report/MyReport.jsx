...
const StatCard = ({ title, value, icon, color, subtitle }) => (
  ...
  >
    <div className="flex items-center justify-between">
      ...
      <div
        className={`w-4 h-4 bg-opacity-20 rounded-full flex items-center justify-center flex-shrink-0 ${color.replace("border-", "bg-").replace("-500", "-100")}`}
      >
        {icon}
      </div>
    </div>
...
export const MyReport = () => {
  ...
  const calculateStats = useCallback(() => {
    ...
    return {
      ...
      budgetUsed,
      categoryChartData,
      monthlyChartData: monthlyChartData.length > 0 ? monthlyChartData : allMonthlyChartData,
      allMonthlyChartData,
      transactionCount: allTransactions.length,
      savingsProgress,
      totalSavingsTarget,
  ...
  }, [expenses, incomes, budgets, savings, getAllTransactions, selectedMonth, selectedYear]);
  ...
  // Export report as PDF using jsPDF with autoTable
  const exportPDF = useCallback(() => {
    ...
    try {
      ...
      if (user?.email) {
        doc.setFontSize(9);
        doc.setTextColor(80);
        doc.text(`User: ${user.email}`, margin, 38);
      }
      ...
      const statsData = [
        ...
        ['Net Savings', formatCurrency(stats.savings)],
        ['Budget Usage', `${Math.round(stats.budgetUsed)}%`],
        ['Budget Remaining', formatCurrency(stats.budgetRemaining)],
        ['Savings Progress', `${stats.savingsProgress.toFixed(1)}%`],
        ['Total Transactions', stats.transactionCount.toString()],
        ['Budget Categories', stats.budgetCount.toString()],
        ['Savings Goals', stats.savingsCount.toString()],
      ];
      ...
      if (stats.categoryChartData.length > 0) {
        doc.setFontSize(14);
        doc.setTextColor(41, 128, 185);
        ...
        const categoryData = [
          ['Category', 'Expenses (RWF)', 'Income (RWF)'],
      ...
      // Monthly Trends
      if (stats.monthlyChartData.length > 0) {
        ...
        const trendData = [
          ['Month', 'Income (RWF)', 'Expenses (RWF)'],
          ...stats.monthlyChartData.map((item) => [
            item.month,
            item.income.toFixed(0),
            item.expenses.toFixed(0),
          ]),
      ...
      doc.text(
        'Generated by Household Expenses Management System',
        pageWidth / 2,
        pageHeight - 10,
        { align: 'center' }
      );
  ...
  return (
    <div id="report-content" ref={reportRef} className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 p-3 sm:p-4 md:p-6 lg:p-8">
      {/* Header */}
      <div className="mb-6 sm:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-1 sm:gap-2 px-3 sm:px-4 py-2 bg-red-100 text-red-700 rounded-lg hover:bg-red-200 transition-colors text-sm sm:text-base disabled:opacity-50"
            >
              <PictureAsPdfIcon className="w-4 h-4" />
              <span className="hidden xs:inline">{isExporting ? "Exporting..." : "PDF"}</span>
            </button>
          </div>
        </div>
      </div>
              ...
              <>
                ...
                {selectedChart === "pie" && (
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie
                        data={stats.categoryChartData.filter((d) => d.expenses > 0 || d.income > 0)}
                        dataKey="expenses"
                        nameKey="name"
                        cx="50%"
                        cy="50%"
      ...
      <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        ...
        <motion.button
          ...
          onClick={exportPDF}
          disabled={isExporting}
          className="bg-gradient-to-r from-red-500 to-red-600 text-white rounded-2xl shadow-lg p-4 sm:p-6 text-left hover:shadow-xl transition-all disabled:opacity-50"
        >
          <PictureAsPdfIcon className="w-6 h-6 sm:w-8 sm:h-8 mb-2" />
          <h4 className="font-semibold text-sm sm:text-base">Export PDF</h4>
          <p className="text-xs sm:text-sm opacity-90 mt-1">
            {isExporting ? "Generating..." : "Professional document format"}
          </p>
        </motion.button>
