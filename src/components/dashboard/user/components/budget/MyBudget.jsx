  ...
  ({
    ...
    return (
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        {/* Display User Name - Auto-filled, read-only */}
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">
            User Name
          </label>
...
export const MyBudget = () => {
  ...
  // ============================================================
  // CALCULATE STATS - MUST BE DEFINED BEFORE loadBudgets
  // ============================================================
  const calculateStats = useCallback((budgetData) => {
    const totalAllocated = budgetData.reduce(
      (sum, b) => sum + (b.allocatedAmount || 0),
  ...
  // ============================================================
  // LOAD BUDGETS - NOW calculateStats IS DEFINED
  // ============================================================
  const loadBudgets = useCallback(async () => {
    if (isLoadingRef.current) return;
    ...
    try {
      ...
      // Always filter by userId for non-admin users
      if (!isAdmin && userId) {
      ...
      // For admin, apply user name filter
      if (isAdmin && filterUserName) {
        const filterName = filterUserName.toLowerCase();
        filteredData = filteredData.filter((b) => {
          const budgetEmail = b.email || "";
          return budgetEmail.toLowerCase().includes(filterName);
        });
        console.log(`✅ Admin filtered to ${filteredData.length} budgets`);
      }
      ...
      const uniqueEmails = [...new Set(allBudgets.map((b) => b.email).filter(Boolean))];
      console.log("👥 All emails found in budgets:", uniqueEmails);
  ...
  // Handle filter changes - reload budgets
  useEffect(() => {
  ...
  // Handle edit budget - matches Budget model
  const handleEditBudget = async (e) => {
    e.preventDefault();
  ...
  const openEditModal = useCallback(
    (budget) => {
      ...
      setFormData({
        ...
        email: budget.email || getUserEmail() || "",
        userId: budget.userId || getUserId() || "",
      });
      setErrors({ category: "", allocatedAmount: "" });
      setIsEditModalOpen(true);
    },
        ...
        <div className="bg-white rounded-2xl shadow-lg p-4 mb-6">
          <div className="flex flex-col md:flex-row md:items-center md:space-x-4 space-y-3 md:space-y-0">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {/* User Name filter for admin - uses email filtering */}
              {isAdmin && (
                <input
                  type="text"
