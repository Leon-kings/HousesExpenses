...
//   // Load expenses from API using email from params
...
//       console.log(`📤 Making API request to ${url} for userId: ${userId}`);
...
//         toast.info(`No expenses found for this user.`);
//       } else {
//         toast.success(
//           `Loaded ${expenseData.length} expenses successfully`
//         );
//       }
//     } catch (error) {
//       console.error("❌ Load expenses error:", error);
...
//     // Set email from params or user data
//     const userEmail = email || userData.email || "";
//     setFilterEmail(userEmail);
...
//     };
//   }, [email]); // Re-run when email param changes
...
//       date: expense.date ? expense.date.split("T")[0] : new Date().toISOString().split("T")[0],
//       user: expense.user || user?.name || "",
//       email: expense.email || user?.email || email || "",
...
//       50,
//     );
//     doc.text(`User: ${email || user?.email || "N/A"}`, 14, 60);
...
//     doc.save(`hems-my-expenses-${today.toISOString().split("T")[0]}.pdf`);
//     toast.success("PDF Report downloaded successfully!");
//   }, [expenses, filteredExpenses, stats, user, email]);
...
//                       {[filterEmail, filterMonth, filterYear].filter(Boolean).length}
//                     </span>
//                   )}
//                 </button>

//                 {(filterEmail || filterMonth || filterYear || searchTerm || filterCategory !== "all" || filterType !== "all") && (
//                   <button
...
//                     <th className="text-left py-3 px-3 sm:px-4 text-xs sm:text-sm font-semibold">Category</th>
//                     <th className="text-left py-3 px-3 sm:px-4 text-xs sm:text-sm font-semibold">Type</th>
//                     <th className="text-left py-3 px-3 sm:px-4 text-xs sm:text-sm font-semibold">User</th>
//                     <th className="text-left py-3 px-3 sm:px-4 text-xs sm:text-sm font-semibold">Email</th>
//                     <th className="text-left py-3 px-3 sm:px-4 text-xs sm:text-sm font-semibold">User ID</th>
//                     <th className="text-right py-3 px-3 sm:px-4 text-xs sm:text-sm font-semibold">Amount (RWF)</th>
//                     <th className="text-center py-3 px-3 sm:px-4 text-xs sm:text-sm font-semibold">Actions</th>
//                   </tr>
//                 </thead>
//                 <tbody>
        ...
        <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-4 sm:mb-6">
          ...
          <div className="bg-white rounded-xl shadow-lg p-3 sm:p-4 border-l-4 border-blue-500">
            <div className="flex items-center justify-between">
              <div className="min-w-0">
                <p className="text-xs sm:text-sm text-gray-500 truncate">Net Balance</p>
                <p className={`text-base sm:text-xs font-bold truncate ${stats.netBalance >= 0 ? "text-blue-600" : "text-red-600"}`}>
                  {formatCurrency(stats.netBalance)}
                </p>
              </div>
              <AttachMoneyIcon className="w-8 h-8 sm:w-10 sm:h-10 text-blue-500 bg-blue-100 p-1.5 sm:p-2 rounded-full flex-shrink-0" />
